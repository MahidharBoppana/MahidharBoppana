# Hi 👋, I'm Naga Venkata Mahidhar Babu Boppana

### MERN Stack Developer | Full-Stack Web Developer | IMCA Student

<p align="left">
  <a href="https://mahidhar-boppana.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-Visit%20Website-black?style=for-the-badge&logo=vercel" />
  </a>
  <a href="https://www.linkedin.com/in/mahidharboppana/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://leetcode.com/u/mahidharbabuboppana/">
    <img src="https://img.shields.io/badge/LeetCode-Profile-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" />
  </a>
</p>

I'm a Full-Stack Web Developer focused on building modern, responsive, and scalable web applications using the **MERN stack**.

I enjoy working across both frontend and backend development — from designing interactive React interfaces to building REST APIs, authentication systems, and MongoDB-backed applications.

Currently pursuing an **Integrated Master's of Computer Applications (IMCA)** at **Parul University**.

---

## 👨‍💻 About Me

* 🎓 Pursuing **Integrated Master's of Computer Applications (IMCA)**
* 💻 Focused on **MERN Stack & Full-Stack Web Development**
* ⚛️ Building modern applications with **React.js**
* ⚙️ Developing REST APIs using **Node.js & Express.js**
* 🗄️ Working with **MongoDB & Mongoose**
* 🧠 Practicing **Data Structures & Algorithms**
* 🚀 Interested in building production-ready full-stack applications
* 📚 Continuously improving my backend and software engineering skills

---

## 🛠️ Tech Stack

### Languages

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js" />
</p>

### Frontend

<p>
  <img src="https://skillicons.dev/icons?i=react,tailwind,materialui" />
</p>

### Backend

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express" />
</p>

### Database

<p>
  <img src="https://skillicons.dev/icons?i=mongodb" />
</p>

### Developer Tools

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,postman,npm,vite,vercel,netlify" />
</p>

---

## 🚀 Featured Projects

### 📝 NoteFlow

**Full-Stack Note Taking Application**

A full-stack note management application built with the MERN stack featuring secure authentication and complete note management.

**Key Features**

* JWT authentication and protected REST APIs
* Create, read, update, and delete notes
* Search functionality
* Pin and unpin notes
* Soft delete and trash management
* Restore deleted notes
* Permanent note deletion
* MongoDB relationships
* Reusable Express controllers and middleware
* Responsive user interface

**Tech Stack:** React.js • Node.js • Express.js • MongoDB • JWT

[GitHub Repository](https://github.com/MahidharBoppana/NoteFlow) • [Live Demo](https://note-flow-eight-inky.vercel.app/)

---

### 🔎 GitHub Finder

**GitHub Profile Search Application**

A responsive React application that allows users to search and explore GitHub profiles using the GitHub API.

**Key Features**

* GitHub API integration
* Profile search
* Asynchronous data fetching
* React Hooks
* Responsive user interface

**Tech Stack:** React.js • JavaScript • GitHub API

---

### 🌦️ Weather App

**Real-Time Weather Application**

A React-based weather application that provides current weather information for cities around the world.

**Key Features**

* Search weather by city
* Real-time temperature
* Humidity information
* Wind speed
* Current weather conditions
* Responsive interface

**Tech Stack:** React.js • JavaScript • CSS3 • Weather API

---

## 🔥 GitHub Streak

<p align="center">
  <img src="https://streak-stats.demolab.com/?user=MahidharBoppana&hide_border=true" />
</p>

---

## 🧠 Coding Practice

<p align="center">
  <a href="https://leetcode.com/u/mahidharbabuboppana/">
    <img src="https://leetcard.jacoblin.cool/mahidharbabuboppana?theme=light&font=Inter&ext=heatmap" />
  </a>
</p>

I regularly practice **Data Structures, Algorithms, and logical programming problems** to strengthen my problem-solving skills.

---

## 🎯 Current Focus

```text
🌐 Full-Stack Web Development
⚛️ Advanced React
⚙️ Backend Development
🟢 Node.js & Express.js
🍃 MongoDB
🧠 Data Structures & Algorithms
🏗️ Software Engineering Fundamentals
```

---

## 🤝 Connect With Me

<p align="left">

<a href="https://www.linkedin.com/in/mahidharboppana/">
  <img src="https://img.shields.io/badge/LinkedIn-Mahidhar%20Boppana-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="https://github.com/MahidharBoppana">
  <img src="https://img.shields.io/badge/GitHub-MahidharBoppana-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://mahidhar-boppana.vercel.app/">
  <img src="https://img.shields.io/badge/Portfolio-Visit-black?style=for-the-badge&logo=vercel&logoColor=white" />
</a>

<a href="https://leetcode.com/u/mahidharbabuboppana/">
  <img src="https://img.shields.io/badge/LeetCode-mahidharbabuboppana-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" />
</a>

</p>

---

<p align="center">
  <b>Thanks for visiting my profile! 👋</b>
</p>

<p align="center">
  <i>Always learning. Always building.</i>
</p>
